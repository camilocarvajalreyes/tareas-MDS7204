...
class PeriodicKernel:
    ...
    def __call__(self, X, Y):
        return self.sigma**2 * np.exp(-2 * (np.sin(np.pi / self.periodicity * outersum(X,-Y)) / self.length_scale**2) ** 2)
    
    @staticmethod
    def kernel_eval(x,y,sigma,length_scale,periodicity):
        return sigma**2 * np.exp (-2 * (np.sin(np.pi / periodicity * outersum(x,-y)) / length_scale**2) ** 2 )
...
class RBFKernel:
    ...
    def show_hypers(self):
        print('Hyperparámetros Kernel RBF:')
        print(f'\tsigma = {self.sigma}')
        print(f'\tlargo de escala = {self.length_scale}')
    
    def __call__(self, X, Y):
        return self.sigma**2 * np.exp(-0.5 * (outersum(X,-Y) / self.length_scale)**2)
    
    @staticmethod
    def kernel_eval(x,y,sigma,length_scale):
        return sigma**2 * np.exp(-0.5 * (outersum(x,-y) / length_scale)**2)
    ...
    def nlogp(self,x,y,Nx,hypers):
        ...
        Gram = self.kernel_eval(x,x,sigma,length_scale)
        K = Gram + sigma_n**2*np.eye(Nx) + 1e-5*np.eye(Nx)
        (_, logdet) = np.linalg.slogdet(K)
        return 0.5*(y.T@np.linalg.solve(K,y) + logdet + Nx*np.log(2*np.pi))
